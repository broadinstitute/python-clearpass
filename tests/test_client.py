# -*- coding: utf-8 -*-
"""Define the clearpass.client.Client unit tests."""
...
from clearpass.client import Client
...
class TestClient(TestCase):  # pylint: disable=too-few-public-methods
    ...
    def setUp(self):  # pylint: disable=invalid-name
        ...
        self.base_url = "https://cppm.example.org"
        self.client_id = "abc-123"
        self.client_secret = "0longclientsecretthesamelengthasgivenbycppm0"
        self.username = "someuser"
        self.password = "secretpassword"
        self.access_token = "abcdef0123456789abcdef0123456789abcdef01"
...
class TestInit(TestClient):
    ...
    def setUp(self):  # pylint: disable=invalid-name
        ...
        self.token_patcher = mock.patch("clearpass.client.Client._get_token", autospec=True)
        self.mock_token = self.token_patcher.start()
        self.mock_token.return_value = self.access_token

        self.session_patcher = mock.patch("clearpass.client.requests.sessions.Session", autospec=True)
        self.mock_session = self.session_patcher.start()
        # self.mock_session.return_value = mock.Mock(requests.sessions.Session)

    def test_defaults(self):
        """Parameters should be set correctly inside the class using defaults."""
        client = Client(
            base_url=self.base_url, client_id=self.client_id, client_secret=self.client_secret,
        ...
        self.assertEqual(client._Client__base_url, self.base_url)
        self.assertEqual(client._Client__client_id, self.client_id)
        self.assertEqual(client._Client__client_secret, self.client_secret)
        self.assertEqual(client._Client__username, self.username)
        self.assertEqual(client._Client__password, self.password)
        self.assertEqual(client._Client__grant_type, self.default_grant_type)
        self.assertEqual(client._Client__grant_type, self.default_grant_type)
        ...
        self.assertTrue("Authorization" in client._Client__headers)
        self.assertEqual(client._Client__headers["Authorization"], f"Bearer {self.access_token}")
...
#         client = Client(
#             base_url=self.cfixt.base_url, login_uri=self.cfixt.login_uri, username=self.cfixt.username,
#             cert_auth=True, user_crt_file=self.cfixt.user_crt_file, user_key_file=self.cfixt.user_key_file,
#         )
...
#         user_agent = f"clearpass/{test_version} (Python {pyver})"

#         ver_patcher = mock.patch("clearpass.__version__.__version__", test_version)
#         ver_patcher.start()
